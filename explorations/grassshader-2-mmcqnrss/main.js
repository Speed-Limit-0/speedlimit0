...
void main() {
    ...
    // Store original height to preserve blade length
    float originalY = localPos.y;
    
    localPos.x += windVector.x;
    localPos.z += windVector.y; // z in 3D maps to y in 2D noise
    
    // Y drop to maintain blade length
    float dispDist = length(windVector);
    // Prevent mathematical errors if displacement exceeds length
    dispDist = min(dispDist, originalY * 0.99);
    // Use Pythagorean theorem to approximate arc bend (x^2 + y^2 = length^2)
    localPos.y = sqrt(originalY * originalY - dispDist * dispDist);
